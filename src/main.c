...
// Given a astring with a '.' and at least a 1-character suffix
// after the '.', change the sufgfix to be the given character.
...
static char *do_compile(char *filename)
{
  ...
  if (Outfilename == NULL)
  {
    fprintf(stderr, "Error: %s has no suffix, try .fl on the end\n", filename);
    exit(1);
  }
...
static void usage(char *prog)
{
  ...
  fprintf(stderr, "       -o outfile, product the outfile execturable file\n");
  exit(1);
}
...
int main(int argc, char *argv[]) {
  ...
  for (i=1; i<argc; i++) {
    ...
    for (int j=1; (*argv[i] == '-') && argv[i][j]; j++) {
      switch (argv[i][j]) {
        case 'o': outfilename = argv[++i]; break; // Save & skip to next argument
        case 'T': O_dumpAST = 1; break;
        case 'c': O_assemble = 1; O_keepasm = 0;  O_dolink = 0; break;
        case 'S': O_keepasm = 1;  O_assemble = 0; O_dolink = 0; break;
  ...
  while (i < argc)
  {
    ...
    if (!O_keepasm)                 // Remove the assemblt file if  
